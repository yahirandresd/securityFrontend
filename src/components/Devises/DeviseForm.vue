  ...
  <div class="min-h-screen bg-gray-100 p-6">
    <div class="w-full bg-white shadow-lg rounded-lg p-8">
      <h2 class="text-3xl font-bold text-gray-800 mb-6 text-center">
        {{ props.deviseId ? "Editar Dispositivo" : "Crear Dispositivo" }}
      </h2>
      ...
      <form @submit.prevent="submitForm" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="col-span-1 md:col-span-2">
          ...
          >
            {{ isSubmitting ? "Enviando..." : props.deviseId ? "Actualizar" : "Crear" }}
          </button>
        </div>
      </form>
    </div>
...
import { useDeviseStore } from '@/store/DeviseStore'
import { DeviseValidator } from '@/utils/DeviseValidator'

import { User } from '@/models/User'

const props = defineProps<{ deviseId?: number }>()
...
onMounted(async () => {
  try {
    ...
    if (props.deviseId) {
      const res = await deviseStore.getDevise(props.deviseId)
      if (res.status === 200) {
        Object.assign(devise, res.data)
      }
    }
  } catch (error) {
...
const validateField = (field: keyof typeof devise) => {
  const result = DeviseValidator.validateField(field, devise[field])
  if (!result.success) {
...
const validateAllFields = () => {
  Object.keys(devise).forEach((field) => {
    validateField(field as keyof typeof devise)
  })
}
...
const submitForm = async () => {
  ...
  try {
    let res
    if (props.deviseId) {
      res = await deviseStore.editDevise(props.deviseId, devise)
    } else {
      res = await deviseStore.addDevise(devise)
    }

    if ([200, 201].includes(res.status)) {
      Swal.fire({
        title: 'Éxito',
        text: props.deviseId ? 'Dispositivo actualizado con éxito ✅' : 'Dispositivo creado con éxito ✅',
        icon: 'success',
        timer: 3000,
